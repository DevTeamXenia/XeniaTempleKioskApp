...
import com.xenia.churchkiosk.data.network.model.Company
...
class SessionManager(context: Context) {
    ...
    fun saveCompanyDetails(company: com.xenia.churchkiosk.data.network.model.Company) {
        val gson = Gson()
    ...
    fun getCompanyDetails(): com.xenia.churchkiosk.data.network.model.Company? {
        val gson = Gson()
        val companyJson = sharedPreferences.getString(KEY_COMPANY_OBJECT, null)
        return if (companyJson != null) {
            gson.fromJson(companyJson, com.xenia.churchkiosk.data.network.model.Company::class.java)
        } else {
            null
        }
    }
